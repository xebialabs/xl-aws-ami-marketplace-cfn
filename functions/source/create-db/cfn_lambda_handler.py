...
def sanitize(response, secure_attributes):
    if response.get('Data'):
        ...
        sanitized['Data'] = {
            k: '*******' if k in secure_attributes else v for k, v in sanitized['Data'].items()}
...
def cfn_handler(func, base_response=None, secure_attributes=[]):
    def decorator(event, context):
        ...
        logger.debug("Received %s request with event: %s" % (
            event['RequestType'], json.dumps(event, default=date_handler)))
        ...
        try:
            if timeout:
                ...
                if int(time.time()) > finish:
                    logger.info(
                        "Function reached maximum timeout of %d seconds" % timeout)
                    response.update({
                        "Status": FAILED,
            ...
            logger.info(
                "Function approaching maximum Lambda execution timeout...")
            ...
            try:
                ...
                logger.exception(
                    "Failed to invoke new Lambda function after maximum Lambda execution timeout: " + str(e))
                response.update({
                    "Status": FAILED,
...
class Handler:
    ...
    def create(self, func):
        self._handlers['Create'] = self._decorator(
            func, secure_attributes=self._secure_attributes)
        return func
